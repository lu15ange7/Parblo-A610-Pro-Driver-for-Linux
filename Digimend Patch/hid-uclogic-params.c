				      ...
				      struct hid_device *hdev)
{
	...
	desc_copy_ptr = kmemdup(uclogic_rdesc_parblo_a610pro_fixed0_arr, uclogic_rdesc_parblo_a610pro_fixed0_size, GFP_KERNEL);
	...
	desc_copy_ptr = NULL;
	pen->desc_size = uclogic_rdesc_parblo_a610pro_fixed0_size;
	found = true;
finish:
	*pfound = found;
	rc = 0;
...
#define VID_PID(_vid, _pid) \
	...
	case VID_PID(USB_VENDOR_ID_UGEE,
		...
		break;
			case VID_PID(USB_VENDOR_ID_UCLOGIC,
		     USB_DEVICE_ID_UGEE_PARBLO_TABLET_A610PRO):
		...
		if (bInterfaceNumber == 0){
			rc = uclogic_params_pen_parblo_init(&p.pen, &found, hdev);
				...
				if (found) {
					rc = uclogic_params_frame_init_with_desc(
						&p.frame_list[0],
						uclogic_rdesc_parblo_a610pro_frame_arr,
						uclogic_rdesc_parblo_a610pro_frame_size,
					UCLOGIC_RDESC_PARBLO_A610PRO_FRAME_ID);
				}
			}
		break;
	case VID_PID(USB_VENDOR_ID_UGEE,
